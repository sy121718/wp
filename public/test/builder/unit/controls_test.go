...
func TestValidateSpec(t *testing.T) {
	cases := []struct {
		...
		{"safe 协议相对外联 url 注入", `{"shade":"url(//evil.com/x.jpg)"}`, "值非法"},
		{"safe https 外联 url 注入", `{"shade":"url(https://evil.com/x.jpg)"}`, "值非法"},
		{"safe 大写 URL 外联变体注入", `{"shade":"URL(https://evil.com/x.jpg)"}`, "值非法"},
		{"safe 合法函数值 rgba 保留", `{"shade":"rgba(0,0,0,0.4)"}`, ""},
		{"safe 站内相对 url 保留", `{"shade":"url(/img/bg.jpg)"}`, ""},
